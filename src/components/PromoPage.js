...
import { Button,Box, Typography, CircularProgress, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Select, MenuItem, FormControl, InputLabel, Tabs, Tab, TextField, Grid, List, ListItem, ListItemIcon, ListItemText, Avatar } from '@mui/material';
...
export default function PromoPage() {
  ...
  useEffect(() => {
    const fetchAbsences = async () => {
      ...
      const promoNames = [...new Set(absencesData.map(absence => absence.etudiant.promo))];
  ...
  useEffect(() => {
    if (selectedPromo) {
      setPromoAbsences(absences.filter(absence => absence.etudiant.promo === selectedPromo));
    }
  ...
  useEffect(() => {
    if (selectedPromo) {
      const promoAbs = absences.filter(absence => absence.etudiant.promo === selectedPromo);
      ...
      promoAbs.forEach(absence => {
        ...
        if (studentAbsences[studentId]) {
          ...
          studentAbsences[studentId] = { id: absence.etudiant.idEtu, name: absence.etudiant.nomEtu, count: 1 };
        }
      });
  ...
  const deleteAbsence = async (idAbs) => {
    ...
    .then(() => {
      ...
      setPromoAbsences(promoAbsences.filter(absence => absence.idAbs !== idAbs));
    });
  }

  return (
    <Box sx={{ width: '100%' }}>
      <Typography variant="h6" sx={{ mb: 2 }}>Statistiques d'absences par promotion</Typography>
      <FormControl fullWidth>
        ...
        >
          {promos.map((promo, index) => (
            <MenuItem key={index} value={promo}>{promo}</MenuItem>
          ))}
        </Select>
      </FormControl>
      <Box sx={{ mt: 2 }}>
            ...
            {currentTab === 0 && (
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {promoAbsences.filter(absence =>
                      absence.etudiant.nomEtu.toLowerCase().includes(searchValue.toLowerCase()) ||
                      absence.matiere.nomMat.toLowerCase().includes(searchValue.toLowerCase())
                    ).map((absence, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
      <Button onClick={() => deleteAbsence(absence.idAbs)} variant="outlined" color="error">Supprimer</Button>
    </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
